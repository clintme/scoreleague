 ...
 * return { state } will return the final state value.
 */
const matchScheduleSelector =(item) => {
  let lists = item.reduce((curr, next) => {
    if (!curr[next.id]) { 
      curr[next.id] = {
        id: next.id,
    ...
    curr[next.id].match.push(next.name);
...
export const matchScheduleDataSelector = createSelector(
